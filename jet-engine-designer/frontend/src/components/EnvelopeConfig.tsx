...
function NumInput({
  ...
  return (
    <input
      type="number"
      className="w-full bg-slate-700 border border-slate-600 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
      value={value}
      min={min}
      max={max}
      step={step ?? 1}
    ...
    <div>
      ...
      >
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Min Speed [km/h]</label>
            <NumInput value={config.speedMinKmh} onChange={v => set({ speedMinKmh: v })} min={50} max={2000} step={50} />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Max Speed [km/h]</label>
        ...
        <div className="mt-3">
          <label className="text-xs text-slate-400 mb-1 block">Fixed Altitude [m]</label>
          <NumInput value={config.altitudeM} onChange={v => set({ altitudeM: v })} min={0} max={20000} step={500} />
          <div className="text-xs text-slate-500 mt-1">Cruise altitude: {formData.cruise_altitude_m} m</div>
        </div>
      </Section>
      ...
      >
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Min Altitude [m]</label>
            <NumInput value={config.altitudeMinM} onChange={v => set({ altitudeMinM: v })} min={0} max={20000} step={500} />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Max Altitude [m]</label>
        ...
        <div className="mt-3">
          <label className="text-xs text-slate-400 mb-1 block">Fixed Speed [km/h]</label>
          <NumInput value={config.speedKmh} onChange={v => set({ speedKmh: v })} min={50} max={3000} step={50} />
          <div className="text-xs text-slate-500 mt-1">Cruise speed: {formData.cruise_speed_kmh} km/h</div>
        </div>
      </Section>
      ...
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-lg transition-colors"
      >
        {loading ? (
