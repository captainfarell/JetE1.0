...
const SECTION_STYLE: Record<string, { bg: string; border: string; text: string }> = {
  'Intake':        { bg: 'bg-app-muted',    border: 'border-app-secondary', text: 'text-app-text'   },
  ...
  'Exhaust':       { bg: 'bg-app-muted',    border: 'border-app-secondary', text: 'text-app-text'   },
};

const DEFAULT_STYLE = { bg: 'bg-app-muted', border: 'border-app-secondary', text: 'text-app-text' };
        ...
        {sections.map((section, i) => {
          ...
          return (
            <React.Fragment key={section}>
              ...
              {i < sections.length - 1 && (
                <span className="text-app-secondary text-sm select-none">→</span>
              )}
            </React.Fragment>
          );
        })}
